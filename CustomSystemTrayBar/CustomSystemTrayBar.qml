...
BasePill {
    ...
    // Custom: icon size and spacing come from PluginService, under the key "SortedSystemTray"
    // (a leftover from when this plugin did regex sorting; changing it orphans the saved values).
    property real configuredIconSize: PluginService.loadPluginData("SortedSystemTray", "iconSize", 18)
    ...
    // PluginService.pluginDataChanged carries only the plugin id, so reload both values.
    Connections {
        target: PluginService
        function onPluginDataChanged(pluginId) {
            if (pluginId !== "SortedSystemTray")
                return;
            root.configuredIconSize = PluginService.loadPluginData("SortedSystemTray", "iconSize", 18);
    ...
    // Custom: auto-overflow / "Keep in Bar" stays off. Pinning it false keeps
    // automaticVisibleItemLimit at the full list, empties autoOverflowBarItems and makes
    // isAutoOverflowTrayItem() always false, so every upstream branch that would introduce it
    // is dead without having to carve the feature out of the file.
    readonly property bool useAutomaticOverflow: false
    property int configuredMaxVisibleItems: SettingsData.widgetOption("systemTray", widgetData, "trayMaxVisibleItems")
    readonly property real configuredIconSpacing: spacerValue(iconSpacingPreset)
    ...
    // Custom: configurable icon size instead of Theme.barIconSize, and the cell is the icon
    // (upstream pads it by spacingXS + spacingXXS) since spacing is a separate setting.
    readonly property real trayIconSize: root.configuredIconSize
    readonly property real trayItemSize: trayIconSize
    ...
    Component {
        ...
        Item {
            ...
            BarPillSurface {
                ...
                color: inlineTrayItemArea.containsMouse || inlineTrayItemArea.pressed ? Theme.primaryHover : Theme.withAlpha(Theme.primaryHover, 0)
    ...
    Component {
        ...
        Item {
            ...
            BarPillSurface {
                ...
                color: trayItemArea.containsMouse || trayItemArea.pressed ? Theme.primaryHover : Theme.withAlpha(Theme.primaryHover, 0)
